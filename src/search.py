...
_SYNONYM_GROUPS: list[set[str]] = [
    ...
    {"部署", "发布", "上线", "发布", "更新", "升级", "安装", "配置"},
    {"清理", "删除", "清除", "去重", "压缩", "熵增", "回收", "清理"},
...
def _field_score(query_tokens: list[str], text: str, use_synonyms: bool = True) -> float:
    ...
    支持同义词扩展：当 use_synonyms=True 时，query token 可以通过同义词命中。
    """
    if not text:
        return 0.0
    ...
    if use_synonyms:
        # 用同义词集匹配：query token 或它的同义词命中 target 就算命中
        hits = 0
        for qt in query_tokens:
            qt_expanded = _expand_synonyms([qt])
            if qt_expanded & target_set:
                hits += 1
