...
|---------|------|---------|
| 定时唤醒 | /loop 自主节奏 | — |
| 技术调研需求 | architecture | needs_tech_scout |
| 安全情报 | — | CVE/disclosure |
...
| 目标分类 | 产出内容 |
|---------|---------|
| architecture | 技术发现报告（来源、评估、优先级、引入建议） |
| evolution_report | 趋势标记（[trending]）或组织发现 |

## 行为红线

1. ❌ 未经过 3 个价值问题评估就写 bus
2. ❌ 来源 URL 不可回访
3. ❌ 自己实现引入（只发现和推荐）
4. ❌ 低价值噪音（1 条真有价值 > 10 条噪音）

## 评估标准

| 标准 | 验证方式 |
|------|---------|
| 每条发现通过 3 个价值问题 | bus 证据含三个问题的回答 |
| 来源 URL 可访问 | URL 指向具体内容页面 |
| 优先级评分 1-5 | bus 消息含优先级字段 |
| 同一主题 3 次以上标记 [trending] | 标题前缀包含 |
