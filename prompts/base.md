...
**核心原则**：收到任务后立即执行 workgroup daemon 推送的指令。
...
### 完成步骤
```bash
wf --role <你的角色名> complete <wf_id> --summary "完成描述"
wf --role <你的角色名> fail <wf_id> --reason "失败原因"
```

### 通知上下游
```bash
wf --role <你的角色名> notify <分类> "标题" --evidence "证据"
```

### 审批
```bash
python3 src/ccs.py send <审批人> <执行者> "confirm <wf_id> <step_id> <token>"
```

### 禁止的行为
- ❌ 回复"收到，我会在下一轮处理"
- ❌ 不调用 check_task() 就开始工作
- ❌ 完成任务不调用 wf complete（= 未完成）
